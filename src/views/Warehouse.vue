    ...
    <div class="container">
      ...
      <div class="main-content">
        ...
        <div class="logs">
          ...
          <button class="btn" @click="toggleExpand">
            {{ isExpanded ? "收起" : "展开" }} 日志
          </button>
...
const loadInventory = () => {
    const storedInventory = localStorage.getItem('inventoryParts');
    if (storedInventory) {
...
const saveInventory = () => {
    localStorage.setItem('inventoryParts', JSON.stringify(inventory.value));
};

const saveLogs = () => {
    localStorage.setItem('inventoryLogs', JSON.stringify(logs.value));
};
